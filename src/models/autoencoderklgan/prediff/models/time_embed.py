...
from .openaimodel import Downsample, Upsample
from .utils import apply_initialization, conv_nd
...
class TimeEmbedResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            nn.GroupNorm(num_groups=norm_groups if channels % norm_groups == 0 else channels, num_channels=channels),
            nn.SiLU(),
        ...
        self.out_layers = nn.Sequential(
            nn.GroupNorm(
                num_groups=norm_groups if self.out_channels % norm_groups == 0 else self.out_channels,
