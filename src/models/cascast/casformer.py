...
class CrossBlock(nn.Module):
    ...
    def __init__(self, hidden_size, cross_heads, split_num, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.cross_attn = CrossAttention(
            ...
            heads=cross_heads,
            dim_head=hidden_size // cross_heads,
            dropout=0,
        )
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
    ...
    def forward(self, latent_x, enc_x, c):
        ...
        x = x + gate_msa.unsqueeze(1) * self.cross_attn(
            x=modulate(self.norm1(x), shift_msa, scale_msa), context=latent_x
        )
        x = x + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
        return x
...
DiT_models = {"DiT-custom": DiT_custom}
